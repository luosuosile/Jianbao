...
public class MainActivity extends BaseActivity {
    ...
    private SharedPreferences sharedPreferences;//一个存储类
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DaoMaster.DevOpenHelper devOpenHelper = new DaoMaster.DevOpenHelper(getApplicationContext(), "theme.db", null);//创建数据库
        DaoMaster daoMaster = new DaoMaster(devOpenHelper.getWritableDb());//获取一个daomaster对象
        DaoSession daoSession = daoMaster.newSession();//获取一个nersession类
    ...
    private void saveUserInfo() {
        ...
        if (accessToken != null && accessToken.isSessionValid()) {
            ...
            HttpUtil.getUserInfo(params, new OnGetListener() {//这个HTTPUTIL只是用来读取成功登录之后的信息而已
                @Override
                public void onNext(Object object) {
                    ...
                    editor.apply();//这样就完成了转换，没那么方便吧，这个是通过更换Constants离得常量来进行修改
                    drawerRecyclerAdapter.notifyItemChanged(0);//还可以控制哪个item changge
                }
            });
        }
    }//保存已登录用户
